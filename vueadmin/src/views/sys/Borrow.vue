    ...
    >
      ...
      <el-table-column prop="status" label="租借状态" width="220">
        <template slot-scope="scope">
          ...
          <el-tag size="small" v-if="scope.row.repairsStatus === 0"
            >待修复</el-tag
          >
        </template>
      </el-table-column>
      ...
      <el-table-column prop="icon" label="操作">
        <template slot-scope="scope">
          <el-button
            type="text"
            v-if="hasAuth('sys:borrow:approve') && scope.row.status === 0"
            @click="editHandle(scope.row.id)"
            >审批</el-button
          >
          <el-button
            type="text"
            v-if="hasAuth('sys:borrow:approve') && scope.row.status === 3"
          ...
          <el-button
            type="text"
            v-if="hasAuth('sys:borrow:approve') && scope.row.status === 3"
            @click="bad(scope.row.id, scope.row.userid)"
            >损坏</el-button
          >
          <el-divider
            v-if="scope.row.repairsStatus === 0 && scope.row.status === 3"
            direction="vertical"
          ></el-divider>
          <el-button
            type="text"
            v-if="
              hasAuth('sys:borrow:approve') && scope.row.repairsStatus === 0
            "
...
export default {
  ...
  methods: {
      ...
      this.$axios
        ...
        .then((res) => {
          ...
          for (let i = 0; i < this.tableData.length; i++) {
            this.tableData[i].starttime = this.dateFormat(
              this.tableData[i].starttime
            );
            this.tableData[i].endtime = this.dateFormat(
    ...
    submitForm() {
      ...
      this.$axios
        .post("/sys/borrow/passOrNotPass", this.editForm)
        .then((res) => {
          this.$message({
            showClose: true,
            message: "恭喜你，操作成功",
