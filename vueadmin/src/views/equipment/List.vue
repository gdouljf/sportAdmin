<template>
  <div>
    <el-form :inline="true">
      ...
      <el-form-item>
        <el-button
          ...
          v-if="hasAuth('sys:equipment:add')"
          >新增器材</el-button
        >
      </el-form-item>
    ...
    >
      ...
      <el-table-column prop="total" label="器材总数"> </el-table-column>
      <el-table-column prop="money" label="器材金额(元/每个每天)">
      </el-table-column>
      ...
      <el-table-column prop="icon" width="260px" label="操作">
        <template slot-scope="scope">
          <el-button
            type="text"
            v-if="hasAuth('sys:equipment:update')"
  ...
  data() {
    return {
      ...
      editFormRules: {
        ...
        total: [{ required: true, message: "请输入器材总数", trigger: "blur" }],
        money: [{ required: true, message: "请输入器材金额", trigger: "blur" }],
        status: [
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$axios
            .post(
              "/sys/equipment/" + (this.editForm.id ? "update" : "add"),
              this.editForm
            )
            .then((res) => {
              this.$message({
