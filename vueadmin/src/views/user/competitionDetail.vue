<template>
  <div style="text-align: center">
    <div style="text-align: left; width: 1080px; margin: 0 auto">
      ...
      <div class="hd">
        <h3
          @click="changeToPeople()"
          :class="isShow ? 'tran' : 'blue'"
          id="details"
        >
          竞赛信息
        </h3>
        ...
        <div style="float: right" v-if="new Date() < new Date(info.starttime)">
          <el-button
            v-if="
              new Date() < new Date(info.starttime) && info.applyStatus === 1
            "
            class="baoMing"
            @click="apply()"
            >立即报名</el-button
        ...
        <div class="apply">
          <span v-if="new Date() < new Date(info.starttime)"
            >报名时间&nbsp;</span
          >
          <span v-if="new Date() < new Date(info.starttime)"
            >距离报名截止还有
            {{
              parseInt(
                (new Date(info.starttime).getTime() - new Date().getTime()) /
                  (1000 * 60 * 60 * 24)
              )
            }}
            天</span
          ...
          <p>
            {{ dateFormat(new Date()) + " - " + dateFormat(info.starttime) }}
          </p>
        </div>
      </div>
      <div class="content-left">
        ...
        <div v-if="isShow" class="people">
          <div
            v-if="applyPeopleList.length > 0"
            v-for="apply in applyPeopleList"
          >
            <span>
...
export default {
  ...
  methods: {
    ...
    getApplyPeopleList() {
      this.$axios
        .get("/competition/getApplyPeopleList/" + this.competitionId)
        .then((res) => {
    ...
    cancel() {
      this.$confirm("此操作将取消报名该比赛, 是否继续?", "取消报名", {
        ...
        this.$axios
          .post("/user/apply/cancel/" + this.competitionId + "/" + this.userId)
          .then((res) => {
            this.$message({
              type: "success",
              message: "取消报名成功!",
