  ...
  <div>
    <div v-for="competition in this.competitionList" class="competition">
      ...
      <div style="display: inline-block">
        <h3>
          <el-tag
            v-if="new Date() < new Date(competition.starttime)"
            type="warning"
            >报名中...</el-tag
          >
          <el-tag
            v-if="
              new Date(competition.starttime) <= new Date() &&
              new Date() <= new Date(competition.endtime)
            "
            type="success"
            >比赛中...</el-tag
          >
          <el-tag v-if="new Date() > new Date(competition.endtime)"
            >比赛结束</el-tag
          >
          <a @click="toDetail(competition.id)" style="cursor: pointer">{{
            competition.name
          }}</a>
        </h3>
        <p>
        ...
        <p>
          ...
          <span>{{
            dateFormat(competition.starttime) +
            " ~ " +
            dateFormat(competition.endtime)
          }}</span>
        </p>
      </div>
      <div class="info">
        ...
        <el-button
          v-if="
            new Date() < new Date(competition.starttime) &&
            competition.applyStatus === 0
          "
          @click="toDetail(competition.id)"
          >取消报名</el-button
        >
        <p v-if="new Date() < new Date(competition.starttime)">
          距离报名截止还有
          {{
            parseInt(
        ...
        <p v-else>报名结束</p>
        <p>
          <span style="cursor: pointer" @click="toDetail(competition.id)"
            >竞赛详情 ></span
          >
        </p>
      </div>
...
export default {
  ...
  methods: {
    ...
    getUserCompetitionList() {
      this.$axios
        .get("/competition/getCompetitionList/" + this.userId)
        .then((res) => {
